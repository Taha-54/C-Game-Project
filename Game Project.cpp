...
void DrawTitle() {
    string title = " TAHA'S CONSOLE SHOOTER' ";
    int start = 3;
    ...
    for (int y = 0; y < HEIGHT; y++) {
        ...
        for (int x = 1; x < bufferSize.X - 1; x++) {
            if (x - 1 == playerX && y == playerY) {
                WriteToBuffer(x, y + 3, PLAYER, 11); // Player color changed to blue
                continue;
            }
            ...
            for (auto& e : enemies) {
                if (e.first == x - 1 && e.second == y) {
                    WriteToBuffer(x, y + 3, ENEMY, 13); // Enemy color changed to magenta
                    enemyDrawn = true;
                    break;
                }
            }
...
void SpawnNewWave() {
    wave++;
    enemySpeed = max(3, enemySpeed - 1);
    enemyShootDelay = max(5, enemyShootDelay - 2);
...
int main() {
    ...
    SetConsoleTitleA("TAHA'S SHOOTING ARENA GAME");
